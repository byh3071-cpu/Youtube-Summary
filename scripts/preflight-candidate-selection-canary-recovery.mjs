...
const expected = {
  ...
  failure_code: "NLM_DRAFT_CONTRACT_INVALID",
  failure_message: "NotebookLM 응답의 문자열 필드 길이가 올바르지 않습니다.",
};
...
if (!row.metadata?._semantic_json_fence_recovery_v1 || !row.metadata?._public_caption_config_recovery_v1) {
  fail("Canary is missing a prerequisite recovery marker.");
}
if (row.metadata?._candidate_selection_format_recovery_v1) fail("Canary was already recovered by 021.");
console.log(JSON.stringify({
  ...
  row: {
    ...
    candidate_selection_marker_present: false,
  },
}, null, 2));
