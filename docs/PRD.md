...
| 후속 전이 | `review_required → approving → completed`는 Control Tower의 사람 결정을 받은 yohan-mcp 승인 명령이 맡는다. DB RPC가 approval token·intent hash를 CAS하고, 사람 메모는 Git 제외 로컬 intent에만 고정해 RESOURCE/SUMMARY pair의 중간 crash를 복구한다. `action_required`·`failed`는 인증·자막·접근 제한·한도·처리 지연별 다음 행동을 `/knowledge`에 표시한다. 조치 후 재처리는 service-role 전용 `015_knowledge_job_retry.sql` RPC와 `knowledge retry <job-id>`만 사용하며, 허용된 실패·3회 미만만 queued로 돌리고 기존 NotebookLM source ID·hash는 보존한다. 강화 이전 엔진이 만든 불완전한 검토는 `018_knowledge_review_invalidation.sql`의 고정 사유 RPC로 먼저 `action_required`에 격리한 뒤 같은 retry 경로만 사용한다. 이미 3회에 도달한 레거시 검토에는 metadata marker를 남기고 단 한 번만 마지막 교정 시도를 허용해 무한 재처리를 막는다. `019_knowledge_semantic_json_fence_canary_recovery.sql`은 PR #67 이전의 strict JSON 호환성 결함으로 3회에 도달한 고정 canary UUID 3개만 exact failure·source/hash 조건과 단회 marker로 한 번 복구하며 일반 재시도 한도를 넓히지 않는다. 처리 준비가 끝난 기존 job은 버튼을 비활성화하고, 중단된 예약은 [처리 준비 다시 시도]로 같은 행을 복구한다. |
...
| 적용 | `012_knowledge_jobs.sql`은 새 설치용 기준선이다. 운영 이력은 legacy 설치에 `014`·`015`·`016`·`017`·`018`을 순서대로 적용했고, 2026-08-12 기준 canary 성공 누계는 `completed 3 + review_required 4 = 7`이다. Brain 동일 job 재승인은 idempotent 응답과 RESOURCE/SUMMARY 해시 불변으로 정확히 1회 적재를 확인했다. yohan-mcp PR #67은 NotebookLM의 단일 JSON 코드펜스를 exact contract 안에서 허용하도록 병합됐다. `019`는 운영 DB 미적용 상태이며 사람 승인 전에는 함수 설치·실데이터 복구를 모두 실행하지 않는다. 적용 직전에는 실제 처리에 사용할 worker checkout이 PR #67 head `8c4e08d` 또는 그 descendant임을 read-only로 확인하고 해당 checkout의 semantic JSON fence 회귀 테스트를 통과시킨다. 이어 `node scripts/preflight-semantic-canary-recovery.mjs`가 세 행의 exact failure·source identity/hash와 marker 미사용을 다시 확인한 뒤 한 행씩 복구한다. |
