...
| 적용 | `012_knowledge_jobs.sql`은 새 설치용 기준선이다. 운영 이력은 legacy 설치에 `014_knowledge_jobs_legacy_upgrade.sql`과 `015_knowledge_job_retry.sql`을 적용한 뒤 `016`·`018` 계약을 포함한 현재 RPC 시그니처·권한을 read-only preflight로 확인하고, 2026-08-10 사람 승인으로 `017_knowledge_jobs_browser_read_hardening.sql`을 적용해 `anon`·`authenticated`의 원본 table SELECT를 닫은 상태다. 기존 NotebookLM source를 재사용한 canary는 `review_required`까지 처리했지만, 이번 출시에서 `018`의 실데이터 invalidation·재처리나 Brain 적재는 실행하지 않는다. Brain 적재는 항목별 사람 승인 전까지 계속 막는다. |
