...
| 적용 | `012_knowledge_jobs.sql`은 새 설치용 기준선이다. 운영 이력은 legacy 설치에 `014_knowledge_jobs_legacy_upgrade.sql`을 적용한 뒤 `015_knowledge_job_retry.sql`을 적용한 것이며, 인증된 worker가 기존 NotebookLM source를 재사용한 canary를 `review_required`까지 처리했다. `016_knowledge_job_approval_cas_hardening.sql`, 기존 authenticated 원본 table SELECT를 닫는 `017_knowledge_jobs_browser_read_hardening.sql`, 레거시 검토 무효화·재처리 사유를 추가하는 `018_knowledge_review_invalidation.sql`은 준비만 됐고 아직 적용하지 않았으며, 이 문서는 현재 live DB 재검증을 주장하지 않는다. Brain 적재는 항목별 사람 승인 전까지 계속 막는다. |
