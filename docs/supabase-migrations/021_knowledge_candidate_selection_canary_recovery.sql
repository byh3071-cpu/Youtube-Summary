-- 021: one-time recovery for the first public-caption canary after
-- yohan-mcp fixed intermittent multi-ID candidate selections in PR #69.
...
create or replace function public.recover_knowledge_candidate_selection_canary(
  p_user_id uuid,
  p_job_id uuid
)
...
begin
  ...
  where job.id = p_job_id
    ...
    and job.failure_code = 'NLM_DRAFT_CONTRACT_INVALID'
    and job.failure_message = 'NotebookLM 응답의 문자열 필드 길이가 올바르지 않습니다.'
    ...
    and coalesce(job.metadata ->> '_candidate_selection_format_recovery_v1', '') = ''
  returning job.* into recovered_job;

  if not found then
    raise exception 'candidate selection canary is not eligible for recovery';
  end if;
  return recovered_job;
end;
$$;

revoke all on function public.recover_knowledge_candidate_selection_canary(uuid, uuid)
  from public, anon, authenticated, service_role;
grant execute on function public.recover_knowledge_candidate_selection_canary(uuid, uuid)
  to service_role;
