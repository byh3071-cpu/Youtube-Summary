...
- 로컬 Phase 커밋은 허용한다.
- 사용자 승인으로 Draft PR #36까지 push했다. production 배포와 main 병합은 별도 승인 전까지 금지한다.
...
| P2 | 롱폼·숏폼·라이브·AI 상태 완료 | preview-verified | 로컬 수용 기준과 Preview 실제 AI 상태 통과 |
| P3 | Preview 인증·Gemini 실검증 | preview-verified | Google OAuth, 상세 복귀, 실제 요약·사용량·캐시 확인 |
...
| P6 | 다중 검수·병합 준비 | preview-verified | 디자인·기능·코드 검수와 전체 게이트 완료, 사용자 병합 승인만 남음 |
...
Preview 보호·툴바가 삽입된 임시 Chrome 세션에서 복구 가능한 React hydration #418이 1회
관찰됐으나, 동일 운영 데이터의 로컬 개발 빌드에서는 console/page error 0건으로 재현되지 않았다.
앱 코드 회귀로 분류하지 않고 Vercel 삽입 UI 관찰 사항으로 남긴다.

## 현재 blocker와 다음 Task

- 구현·로컬 QA·CI·Preview 인증/AI 검증 blocker는 없다.
- Supabase Redirect URLs에는 Vercel Preview용
  `https://*-byh3071-7593s-projects.vercel.app/**`가 등록돼 있다.
- Draft PR #36은 main과 충돌 없이 CLEAN 상태다. 다음 단계는 사용자 최종 승인 후 Draft 해제와
  main 병합이다. production 배포는 별도 승인 전까지 수행하지 않는다.
